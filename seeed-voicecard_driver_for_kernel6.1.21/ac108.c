...
static int ac108_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ac10x == NULL) {
		...
		if (ac10x == NULL) {
			dev_err(&client->dev, "Unable to allocate ac10x private data\n");
			return -ENOMEM;
		}
	}

	index = (int)i2c_match_id(ac108_i2c_id, client)->driver_data;
	if (index == AC101_I2C_ID) {
		ac10x->i2c101 = client;
		i2c_set_clientdata(client, ac10x);
		ret = ac101_probe(client, i2c_match_id(ac108_i2c_id, client));
		if (ret) {
			ac10x->i2c101 = NULL;
			return ret;
		}
	...
	ac10x->i2c[index] = client;
	ac10x->i2cmap[index] = devm_regmap_init_i2c(client, &ac108_regmap);
	if (IS_ERR(ac10x->i2cmap[index])) {
		ret = PTR_ERR(ac10x->i2cmap[index]);
		dev_err(&client->dev, "Fail to initialize i2cmap%d I/O: %d\n", index, ret);
		return ret;
	}
	...
	ac10x_fill_regcache(&client->dev, ac10x->i2cmap[index]);

	ac10x->codec_cnt++;
	pr_info(" ac10x codec count  : %d\n", ac10x->codec_cnt);

	ret = sysfs_create_group(&client->dev.kobj, &ac108_debug_attr_group);
	if (ret) {
...
__ret:
	...
	|| (ac10x->i2c[0] && ac10x->i2c[1] && ac10x->i2c101)) {
		...
		if (ret < 0) {
			dev_err(&client->dev, "Failed to register ac10x codec: %d\n", ret);
		}
	}
	return ret;
}
