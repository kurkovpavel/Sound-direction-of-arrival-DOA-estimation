...
static int wm8960_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct wm8960_data *pdata = dev_get_platdata(&client->dev);
	...
	wm8960->mclk = devm_clk_get(&client->dev, "mclk");
	...
	if (pdata)
		...
		wm8960_set_pdata_from_of(client, &wm8960->pdata);
	...
	if (wm8960->pdata.shared_lrclk) {
		ret = regmap_update_bits(wm8960->regmap, WM8960_ADDCTL2,
					 0x4, 0x4);
		if (ret != 0) {
			dev_err(&client->dev, "Failed to enable LRCM: %d\n",
				ret);
			return ret;
		}
	}
