...
#define simple_util_dai asoc_simple_dai
#define snd_soc_rtd_to_cpu(rtd, i) asoc_rtd_to_cpu(rtd, i)
#define snd_soc_rtd_to_codec(rtd, i) asoc_rtd_to_codec(rtd, i)
...
#define simple_util_parse_clk(dev, node, simple_dai, dlc) \
	asoc_simple_parse_clk(dev, node, simple_dai, dlc)  
#endif
...
static int seeed_voice_card_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		...
		if (snd_soc_dai_stream_active(dai, SNDRV_PCM_STREAM_CAPTURE) && substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			break;
		}
